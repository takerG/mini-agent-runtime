...
func NewChatProxyHandler(endpoint string, defaultModel string, client *http.Client) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		upstreamReq, err := NewChatRequestWithContext(r.Context(), endpoint, model, inbound.Message)
		if err != nil {
		...
		// 我们要把 upstreamResp.Body 保持为流，交给 StreamChatContent 边读边转发。
		upstreamResp, err := client.Do(upstreamReq)
		if err != nil {
			http.Error(w, fmt.Sprintf("post chat request: %v", err), http.StatusBadGateway)
			return
		}
		defer upstreamResp.Body.Close()

		if upstreamResp.StatusCode < 200 || upstreamResp.StatusCode > 299 {
			http.Error(w, fmt.Sprintf("chat request failed: %s", upstreamResp.Status), http.StatusBadGateway)
			return
		}
		...
		// 这是 HTTP 代理模式的核心：
		//   上游：本地模型按行返回 JSON
		//   中间：StreamChatContent 解析每行 JSON，取出 message.content
		//   下游：把 content 立即写给 HTTP 调用方，并在每个 chunk 后 Flush
		if err := StreamChatContent(upstreamResp.Body, w); err != nil {
