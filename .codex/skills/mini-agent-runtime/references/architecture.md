...
- `internal/approval`：Human-in-the-loop 请求、决策、风险画像、审批策略、CLI gate、auto gate，以及 lifecycle/trace 审计适配。
- `internal/tools`：`Tool` interface、`ToolRegistry`、`ExecutionPolicy`、内置工具、参数解析和校验；工具只声明风险，不实现审批交互。
...
- `internal/eval`：JSON eval suite 加载、按 mode 执行用例、基于 trace 检查工具调用、工具结果、lifecycle observation、工具错误和最终答案，并输出 CLI eval 报告。
...
- 新增工具治理策略：风险画像、审批请求和审批 gate 优先放在 `internal/approval`；工具执行策略通过 `internal/tools.ExecutionPolicy` 注入 approval policy；高风险工具通过 `RiskAwareTool` 声明风险，并在 `ExecuteWithPolicy` 中统一触发 Human-in-the-loop。
...
- 新增 eval 能力：优先放在 `internal/eval`，通过现有 `agent.ModeRunner` 和 trace 事件观察真实行为；工具结果和 observation 断言必须读取结构化 trace 数据，不在 eval 中复制 agent 流程，也不解析 stdout 文本。
